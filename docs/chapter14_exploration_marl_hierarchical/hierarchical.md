...
[12.2](./marl) 我们解决了多智能体同时学习时的非平稳性问题，通过 CTDE 范式让多个智能体能协同训练。这一节我们回到单个智能体，但把任务的时间尺度拉长——想象一个家用机器人要完成"打扫整栋房子"的任务，它需要：先去客厅拿吸尘器、再吸卧室、再倒垃圾、最后拖地——这可能需要上千步连续的原子动作。
...
- 底层的**内在奖励**直接定义为"实际位移和目标位移的距离"：$r^l_t = -\|(s_{t+1} - s_t) - g_t\|$——你走得越接近高层给的位移，距离越小，奖励越高；走偏了奖励就低
