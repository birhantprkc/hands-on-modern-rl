...
- **25.4.1 规模、搜索与优化距离**：模型变强为什么不会自动消除奖励漏洞，反而可能放大它。
- **25.4.2 奖励模型过度优化**：代理奖励与独立评价为什么会分叉，以及 Gao 等人的可控实验。
- **25.4.3 五层防线**：奖励、优化、训练、评测、部署五层各自负责什么边界。
- **25.4.4 Alignment Tax**：安全提高的同时原有能力是否退化，退化来自哪里。
- **25.4.5 Inverse Scaling**：更大的模型反而更容易走错路的现象与 U 形曲线。
- **25.4.6 可扩展监督**：监督者看不懂模型的答案时，怎样借助工具或其他模型继续监督。
- **25.4.7 三条仍在发展的研究路线**：Constitutional AI、机制可解释性与形式化验证。
- **25.4.8 防御实验的记录项**：只改变防御层做消融实验时，要保存和报告什么。
- **25.4.9 小结**：收拢本节主线。
