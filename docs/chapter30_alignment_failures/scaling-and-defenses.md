...
本节沿着这条轨迹逐步深入。先解释模型规模、搜索预算和优化距离为什么会放大奖励误差，再看奖励模型过度优化中代理奖励与独立评价怎样分叉。分叉之后，防御要建在奖励、优化、训练、评测、部署五个位置。防御本身也有代价：Alignment Tax 下安全提高可能伴随能力退化，Inverse Scaling 中更大的模型反而更容易走错路。当评测者也开始看不懂模型的答案时，最后看可扩展监督与三条研究路线，并把整套防御落成一次可重复的实验。
