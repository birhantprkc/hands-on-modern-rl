...
这条流水线给出本节的主线：先明确想测什么，再隔离数据与提示，随后固定推理预算和评分器，最后才汇总成分数。任一环节没有版本化，最终数字就无法复现。本节的顺序如下：

- **25.5.1 评测的五个标准**：可验证性、代表性、难度分层、抗污染和统计严谨性。
- **25.5.2 数据污染与泄漏**：显式污染、近似污染与能力迁移，以及各自的检测方法。
- **25.5.3 提示敏感性**：语义等价的提示怎样改变分数与排名，以及多模板标准化。
- **25.5.4 分布外泛化**：输入离开训练分布以后，模型能否继续工作。
- **25.5.5 能力评测与行为评测**：“能不能完成任务”与“在情境里选择什么动作”的区别。
- **25.5.6 长程任务**：几百步轨迹中的终点评估、过程评估与方差问题。
- **25.5.7 AI 研发自动化**：环境配置为什么也是指标的一部分。
- **25.5.8 标准化 Harness**：四类常用评测工具各自覆盖哪一段问题，以及怎样选择。
- **25.5.9 小结**：收拢本节的核心原则。
