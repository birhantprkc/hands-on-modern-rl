...
这条流水线给出本节的主线：先明确想测什么，再隔离数据与提示，随后固定推理预算和评分器，最后才汇总成分数。任一环节没有版本化，最终数字就无法复现。本节按这条流水线展开：先定下评测的五个标准，再依次处理数据污染与泄漏、提示敏感性和分布外泛化，随后区分能力评测与行为评测，进入长程任务与 AI 研发自动化，最后落到标准化 Harness。
