...
本节学习怎样判断一次 RLVR 提升是否可信。检查从验证器与真实任务的分离开始：自动判分也会产生假阳性和假阴性。随后依次看三个来源：数据污染把记忆混进训练曲线，验证器漏洞让测试覆盖和答案抽取器制造高分，计算预算让采样数也能制造“进步”。奖励黑客还会从单一环境泛化成更广的行为，离线指标正常时部署事故仍可能发生。最后给出一套每个检查点都适用的诊断记录。
