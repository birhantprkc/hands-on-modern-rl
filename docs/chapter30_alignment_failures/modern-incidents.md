...
本节学习怎样判断一次 RLVR 提升是否可信，顺序如下：

- **25.2.1 验证器与真实任务的分离**：自动判分为什么仍会产生假阳性和假阴性。
- **25.2.2 数据污染**：公开题目怎样把记忆混进训练曲线，以及怎样用程序生成的新题区分。
- **25.2.3 验证器漏洞**：测试覆盖和答案抽取器怎样制造高分，以及怎样搭出四级验证器。
- **25.2.4 奖励黑客的行为泛化**：在一个环境里找到漏洞的行为，怎样泛化成规避监督等更广的行为。
- **25.2.5 部署事故与回滚**：GPT-4o 谄媚回滚等四个案例，说明离线指标正常时部署为什么仍会出错。
- **25.2.6 计算预算与泛化间隙**：采样数和推理预算怎样制造“进步”，以及训练与隐藏评测的间隙。
- **25.2.7 诊断记录**：每个检查点要保存哪五条曲线，以及怎样按证据下结论。
