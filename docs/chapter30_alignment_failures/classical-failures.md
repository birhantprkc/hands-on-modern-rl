...
本节从这条缝隙开始，逐步区分奖励黑客、规格博弈、谄媚与欺骗。重点不在记住四个术语，而在判断每一种结论需要什么证据。本节的顺序如下：

- **25.1.1 奖励黑客**：用仓库轨迹说明高奖励可以对应零进度，把奖励与任务的分叉写成一般形式。
- **25.1.2 规格博弈**：策略满足字面规则却违背任务意图，以及为什么补一条规则通常不够。
- **25.1.3 谄媚与欺骗的证据层级**：同一个异常行为的多种解释，谄媚和欺骗各需要什么证据。
- **25.1.4 能力增强与突然转折**：能力提高以后，奖励定义里的漏洞为什么会突然变成可执行行为。
- **25.1.5 从奖励漏洞到内部目标**：CoinRun、mesa-optimization、Sleeper Agents 等研究怎样沿同一条证据线扩展。
- **25.1.6 一个最小诊断流程**：遇到“奖励升高、行为不对”的模型时，按什么顺序保存证据。
