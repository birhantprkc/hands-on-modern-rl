...
本节从这条缝隙开始，逐步区分奖励黑客、规格博弈、谄媚与欺骗。重点不在记住四个术语，而在判断每一种结论需要什么证据。我们先用仓库轨迹确认奖励与任务的分叉，再看字面规则怎样被利用，随后区分谄媚与欺骗各自需要的证据。能力增强可能让漏洞突然变成可执行行为，研究问题也从奖励定义扩展到内部目标，最后所有内容会收拢成一份最小诊断流程。
