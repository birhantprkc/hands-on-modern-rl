...
前面把数学题这种简单环境走完了六步——从目标定义到失败回流，每一步看起来都很清晰。但真要把同一套流程搬到代码修复、网页操作、桌面 GUI 这种需要真正和外部环境交互的任务上，立刻就会发现几乎每一步都不够用。一个 8 行的数学证明，环境只需要 20 行 verifier 代码就能判分；但一个中型 Python 项目的代码修复沙箱就要带 200MB 依赖、十几个启动命令、以及完整的 git history，光把环境拉起来就要一分钟。这一节就来讲，当任务从"输出文本答案"变成"在真实环境里做事"，六阶段闭环每一步要扩展哪些内容。

在正式展开扩展内容之前，先把"数学 RL"和"环境型 agent RL"的本质差异说清楚。做数学题（比如 GSM8K、MATH）时，环境状态其实就是题目文本本身——模型拿到 prompt，输出一段推理和最终答案，验证器只需要跑一个答案检查器（比如把 $\boxed{}$ 里的数字和正确答案比一比），整个过程不需要启动任何外部进程，也没有中间状态需要持久化。但面对 SWE-bench 这种代码仓库任务就完全不一样了：同一个任务需要先 `git checkout` 到某个具体 commit、安装 200+ Python 依赖、用 Docker 起一个隔离的沙箱容器、再运行 pytest 才能拿到反馈；每一次从"初始状态"迁移到"可运行状态"的成本可能是几十秒甚至几分钟，中间如果模型修改了 5 个文件、跑了 3 次测试、环境状态已经和初始完全不同。这两类任务的状态复杂度、迁移成本和失败模式完全不在一个量级上，因此六阶段闭环在每一步都要长出新的内容。

理解了这个本质差异后，我们再来看六个阶段分别新增了什么。数学 RL 的环境通常只有题目和答案验证器。代码仓库、网页和桌面应用把每个阶段都扩展了：目标中出现工具和成本约束，数据要携带环境，SFT 要教授操作协议，RL 要处理长轨迹，评测要复现部署条件，失败回流还要保存完整状态。

- **目标定义**新增工具权限、时间预算、长度与安全边界，代表案例有 Apple 端侧约束、OpenAI 工具任务。
- **数据与环境**新增仓库、容器、网页状态、工具和可恢复检查点，代表案例有 MiniMax M2.1、Kimi K3。
- **SFT**新增工具格式、环境观察、动作顺序和停止条件，代表案例有 Qwen-AgentWorld、UI-TARS。
- **RL/偏好优化**新增多轮轨迹奖励、异步经验和领域专家训练，代表案例有 GLM-5.2、DAPO、MiniCPM5 OPD。
- **评测**新增真实环境、独立任务、成本与安全回归，代表案例有 Claude 系统卡、Computer Use。
- **数据回流**新增失败分类、拒绝采样、蒸馏与验证器更新，代表案例有 DeepSeek-R1、Nemotron-Cascade 2。

这些对应关系可以这样读：第一部分是六阶段闭环的每一个阶段，第二部分是环境型任务相比数学 RL 新增的工程要求，第三部分是公开文献中对应做了这些扩展的代表团队。对照着看就知道，每一个团队的贡献其实都是在某一个或几个阶段上解决了数学 RL 不需要面对的新问题。

上面列出的新增内容不能只看"增加了什么名词"，每个阶段新增的内容背后都有具体的工程含义，我们逐个解释：

**目标定义：为什么要有"时间预算"和"安全边界"。** 先看时间预算——数学 RL 里，一道题给模型 10 分钟还是 1 小时，最终解法不会有本质差别，最多就是多展开几步验证。但 agent 任务完全不同：如果只给 10 分钟预算，模型可能只能读 3 个文件、跑 2 次测试、改 1 处 patch；如果给 1 小时，它就可以完整浏览整个仓库结构、写单元测试、反复调试。时间预算直接决定了模型应该学到什么样的"策略风格"——是快速定位直接改，还是稳扎稳打先理解全貌。再看安全边界，这是数学 RL 里几乎不存在的问题：数学题再怎么做，输出的也只是文本；但 agent 任务里模型握着实打实的工具权限，所以必须明确哪些动作绝对不能做——比如不能 `rm -rf /` 删根目录、不能读取用户的 `~/.ssh/` 私钥文件、不能发起对外网络请求访问内网资源、不能在用户没有授权的情况下安装软件。这些安全边界一旦写进目标定义，后面 reward 设计和评测检查都要对应加约束。

讲完目标定义的新增内容，接下来看数据与环境阶段。这个阶段是数学 RL 和 agent RL 工程量差距最大的地方。

**数据与环境："可恢复检查点"到底是什么。** 数学 RL 的一条样本就是 `{"prompt": "...", "answer": "..."}`，丢了也没关系，从原始 JSONL 里再加载一次就行。但环境型任务不一样，一条长任务可能已经跑了 40 分钟、模型改了 12 个文件、执行了 30 次工具调用，这时候 rollout worker 突然崩溃了——如果没有任何中间状态保存，下一次必须从零重新 checkout 仓库、安装依赖、重跑前 30 步，前面 40 分钟的算力全部浪费。可恢复检查点就是为了解决这个问题：它把"当前环境的完整状态"（比如文件系统快照、数据库事务状态、浏览器已打开的标签页和 DOM 状态）定期保存下来，下次崩溃后可以直接从最近的 checkpoint 恢复，不用从头再来。这对百万 token 级的长程研究任务尤其重要。

数据与环境准备好了，接下来是 SFT 冷启动。这一步也不再只是教输出格式。

**SFT："环境观察"和普通 prompt 有什么不同。** 做数学题的 SFT 时，模型看到的输入只有用户 prompt，输出是推理过程。但 agent 任务的 SFT 里，模型面对的输入远不止 prompt——它还包括 `read_file` 返回的文件内容、pytest 跑失败后的红绿色报错堆栈、浏览器工具返回的 DOM 结构和截图描述、`search_code` 工具返回的 JSON 格式搜索结果。这些就是"环境观察"：它们不是用户写的，而是模型上一步动作之后环境返回的真实反馈。SFT 要教模型的不只是工具调用格式，更是"看到 pytest 报 `AttributeError: module 'x' has no attribute 'y'` 之后，下一步应该去哪个文件找定义"这种基于观察的决策逻辑。

SFT 解决了"能开始行动"的问题，接下来 RL 阶段要解决"行动得更好"的问题。这一步的复杂度也上升了一个量级。

**RL/偏好优化："多轮轨迹奖励"和"异步经验"分别解决什么问题。** 先看多轮轨迹奖励：数学 RL 通常是"一次性输出 → 验证对错 → 给 0/1 分"，reward 只在最后给一次。但如果是工具任务，可能一次 episode 有 30 多轮工具交互，reward 要在每一轮和最终交付时分多次给出。agent 任务有 2 步、10 步、50 步等不同长度，每一步动作都会影响后续状态，所以不能只给最后结果一个分——中间如果模型正确读了文件要给小的正反馈、如果连续 3 次调用同一个没有新信息的工具要给负反馈、如果破坏了原有测试要立即扣分。这种"把奖励拆到轨迹不同阶段"的设计就是多轮轨迹奖励。再看异步经验：同步 GRPO 里，同一个 batch 的所有轨迹必须全部跑完才能开始训练，但 agent 任务的耗时差异极大——有的 30 秒就结束了，有的要跑 15 分钟。如果同步等最慢的那一条，前面做完的 GPU 就一直闲置。异步经验的做法是：每条 rollout 做完就立刻放进经验队列，训练端凑够一个 batch 就立即开始更新，不用等组内最慢的那一条，算力利用率显著提升。

RL 训完后，评测阶段也不只是看正确率了。agent 任务的评测要回答一个数学 RL 不需要问的问题：这个模型上线后会不会亏钱、出安全事故？

**评测：为什么还要看"成本与安全回归"。** 数学 RL 评测只看正确率就够了，因为每道题的推理成本差异不大。但 agent 任务上了线就要烧钱。举个具体例子：某一轮训练后，模型的 SWE-bench 通过率从 28% 升到了 35%，看起来是进步了；但评测发现，它平均每个任务要调用 300 次工具、消耗 2 美元的推理成本，而旧模型只需要 50 次调用、0.3 美元。如果上线按这个成本跑，用户用一次平台就亏一次，业务上完全不可接受。安全回归同样重要：比如 RL 训练让模型学会了通过修改 `/etc/hosts` 来绕过某些网络限制，虽然能提高通过率，但这在真实部署里是严重的安全漏洞。所以评测除了看任务完成率，还要监控平均工具调用次数、单次任务成本、敏感操作触发率这些指标。

最后是失败回流阶段。数学 RL 的失败回流就是"把错题补进训练集"，但 agent 任务的回流要复杂得多。

**数据回流："拒绝采样"和"验证器更新"是怎么一回事。** 先看拒绝采样：数学 RL 里通常是"模型输出 → 验证器打分 → 正确的留下、错误的丢弃"这么简单。但环境型任务的轨迹很长，不能简单二元判断——一条 50 步的轨迹，可能前 45 步操作全对，只是最后一步 patch 路径写错了，这种轨迹虽然最终 reward 是 0，但前 45 步的经验非常有价值，不应该整条丢掉。拒绝采样就是更细粒度的筛选：按片段、按轮次、按中间 reward 把轨迹中有价值的部分留下来，而不是非黑即白。再看验证器更新：假设这一轮训练跑完，团队分析失败样本时发现，有一类任务 reward 一直给错——比如模型通过硬编码绕过了测试，但旧 verifier 没查出来，导致很多"假阳性"的高 reward 轨迹混进了训练。这时候要先修验证器的规则，再把这一轮的旧样本用新版 verifier 重新跑一遍、重算 reward，然后再回流到下一轮训练，否则坏信号会一直累积。

把六个阶段的新增内容串起来看，就能得到一张完整的差异对比表。下面把数学 RL 和环境型 Agent RL 在训练系统层面的关键差异整理出来：

::: info 数学 RL vs 环境型 Agent RL：训练系统的关键差异

- **State 复杂度**：数学 RL 的状态就是题目文本本身，几乎没有外部依赖；环境型 Agent RL 的状态包括文件系统、进程、网络、DOM、数据库等，环境迁移成本很高。
- **Reward 延迟**：数学 RL 单次输出后立即给 0/1 分，延迟极低；环境型 Agent RL 需要多轮交互后才有最终结果，中间必须设计过程奖励和 credit assignment。
- **Episode 时长**：数学 RL 的解题长度相对均匀，通常几秒到几十秒完成；环境型 Agent RL 差异极大，从 30 秒到 15 分钟甚至数小时都有，同步等待会造成严重算力浪费。
- **Rollout 失败率**：数学 RL rollout 失败率很低，最多就是答案格式出错；环境型 Agent RL 沙箱启动失败、依赖装不上、工具崩溃、进程超时都是高频问题。
- **样本复用方式**：数学 RL 只要重复加载 prompt 就能复现；环境型 Agent RL 需要版本化环境快照 + 可恢复 checkpoint 才能重放相同状态。

:::

::: details 加餐：为什么数学 RL 不能直接照搬到 Agent 任务

很多团队做数学 RL 拿到了不错的结果后，会想"既然同一套 GRPO 在数学上能涨点，直接用到 agent 任务上应该也行吧？"实际做了才会发现，几乎每一层都要重写。核心差异有四点：

**第一，动作空间从 token 变到工具+环境。** 数学 RL 的动作就是"输出下一个 token"，所有可能的动作都在词表里。但 agent 任务的动作包括调用工具、读文件、跑测试、提交 patch，每一个动作的返回结果取决于真实环境的当前状态，根本无法在词表里穷举。这意味着训练 pipeline 里必须接入真正的工具执行层，而不是一个纯文本的 token 采样器。

**第二，episode 长度从百 token 级变到千 token 级以上。** 一道 GSM8K 数学题通常 200-800 token 就解完了，一次 rollout 成本很低。但一个 SWE-bench 代码修复任务可能要 5000-20000 token，中间还要跑 10-30 次工具。episode 变长以后，attention 计算、KV 缓存管理、rollout 超时策略都要重新设计，原来的 batch 大小和分组策略也完全不适用了。

**第三，reward 延迟更久，credit assignment 更难。** 数学题输出完最后一个 token 就能立刻判 0/1 分。但 agent 任务里，第 5 步读错的文件可能要到第 45 步提交 patch 时才暴露问题；反过来第 3 步做对的关键定位，贡献也要等最终通过测试时才能确认。中间奖励怎么拆、每一步的优势怎么估计，都是数学 RL 不需要面对的新问题。

**第四，验证器需要真正执行环境而不是比对答案。** 数学题的 verifier 只要抽 $\boxed{}$ 里的数字和答案比对就行，20 行代码搞定。但代码修复的 verifier 要能启动沙箱、安装依赖、运行 pytest、检查 patch diff；网页 agent 的 verifier 要能真实点击 DOM、检查页面状态变化；桌面 GUI 的 verifier 还要能截图识别、模拟键鼠操作。验证器本身的代码量、启动成本、维护复杂度和数学题根本不在一个量级上。

:::

## 公开案例

六阶段框架讲完，读者容易遇到一个问题：公开报告里新算法一个接一个，GRPO、DAPO、SAO、OPD……到底该先看哪一个？其实这些名字背后都是对闭环某个环节的具体改进。下面四个案例恰好覆盖了四个最常见的改进入口：MiniCPM5 处理**数据回流**里多专家能力怎么合并，Kimi K3 处理**数据与环境**里 harness 过拟合，Qwen-AgentWorld 处理**SFT + RL** 的训练顺序，GLM-5.2 处理**RL** 训练系统的长短轨迹异步问题。

阅读这四个案例时，先找它改变了闭环中的哪一步，再看用了什么算法名称。

### MiniCPM5

MiniCPM5 改进的是数据回流 + RL 两步，解决的是"专项能力很强但合进通用模型就顾此失彼"的问题。公开流程是 SFT、分领域 RL、OPD 三步。团队先分别训练数学、代码、闭卷问答、写作等领域的 RL 专家模型，再让最终学生模型对同一批领域提示采样，教师在学生生成的每个 token 位置给出概率分布，学生据此调整自己的输出分布。这就是 On-Policy Distillation（OPD）。

它和普通离线蒸馏的差别在数据来源。离线蒸馏先保存教师写好的固定答案，学生反复模仿——相当于老师提前把标准答案印成讲义，学生只管背，考试照着默写；至于学生自己做题时会在哪一步走偏，老师根本不知道。OPD 的回答由当前学生自己生成，教师纠正的是学生此刻真正走到的状态——像老师坐在旁边一对一辅导，学生当场写解法，老师盯着每一步，写错方向就在那个位置实时纠正。两种方式的针对性完全不在一个级别。

工程上 OPD 还有一个容易忽略的优势：领域专家已经在各自领域跑过完整的 SFT + RL，它们训练时用的领域提示（数学竞赛题、代码修复题等）可以直接复用，不需要再为蒸馏单独构造新题目。备课的教案直接拿来用，不用重新写讲义。

从数据工程角度看，这套方法要求每条样本同时记录领域、提示版本、学生策略版本、教师版本和逐 token 的教师信号。OPD 的公式与日志结构放在 [18.5](./data-engineering) 的进阶部分。

从闭环定位看，MiniCPM5 改了两步：一是 RL 阶段先分领域训专家，让每个领域的策略在专项数据上充分收敛；二是数据回流阶段用 OPD 把多个专家蒸馏合并回一个模型，相当于把上一轮多个专家的成功经验合成下一轮的冷启动数据。

### Kimi K3

Kimi K3 改进的是数据与环境一步，解决 agent 训练容易过拟合某一种外层执行框架（harness）的问题——模型学到的是固定格式套路，不是真正的任务决策逻辑。想一下：如果所有训练都用同一个 ReAct 模板，固定思考格式、固定工具调用协议、固定上下文管理方式，模型会学成什么样？它很可能只是记住了"第一步先写 `Thought: 我需要先看一下代码结构`，第二步调用 `list_dir`"这个固定流程，换一种不写 `Thought` 前缀的框架，它就连工具都不会调了。

为了避免这个问题，Kimi K3 把工具、提示、上下文管理、技能、记忆和子智能体组合成统一训练环境。同一个任务可以换用不同工具配置或执行框架——这次用 ReAct + Bash，下次用 Plan-then-Edit + `apply_patch`，再下次用带反思循环的框架，让模型学到"怎么修 bug"本身，而不是"怎么在某一种 harness 里按格式作答"。任务侧还可以从知识图谱选择相关节点，检索公开文章、博客和代码仓库，合成长程研究任务。

Kimi K3 同样先训练不同领域和思考强度的专家，再用多教师 OPD 合并能力。对百万 token 的长任务，系统保存未完成轨迹和外部 KV 状态，下一轮从中间继续，不必每次从头生成。举个直观例子：一个百万 token 研究任务，rollout 已经跑到 80 万 token，模型读完 20 篇论文、整理了引用、写了大半报告，这时候机器宕机——如果没有保存 KV 状态，重跑就要从第 1 个 token 开始，前面几十个小时算力全部浪费；把 KV 缓存持久化到外部存储以后，重启加载最近一次 KV，就能从 80 万 token 位置继续往下写，节省几倍重跑时间。

从闭环定位看，Kimi K3 改了两步：一是数据与环境阶段把"可组合训练环境"本身作为设计重点，通过多 scaffold、多工具配置的组合避免过拟合；二是数据回流阶段沿用 OPD 合并专家，同时增加长任务 KV 状态持久化，让中断的长轨迹也能作为下一轮可用样本。

### Qwen-AgentWorld

Qwen-AgentWorld 改进的是 SFT + RL 的训练顺序，解决直接上手 RL 导致模型连环境反馈都看不懂的问题。它使用容器沙箱、MCP 工具、Android、网页和操作系统模拟器构造任务，训练分为三个阶段：继续预训练让模型学习环境知识，SFT 让模型预测下一状态并学会基本操作，最后用 GSPO 和混合奖励训练完成任务。

三阶段顺序可以用学骑自行车类比：继续预训练相当于先坐在家里读说明书，了解"车把控制方向、脚踏驱动后轮、手刹减速"这些基本原理，知道什么动作会导致什么环境变化；SFT 相当于在停车场由教练扶着，练习上车、握把、踩踏板、刹车这些基本动作，建立"动作 → 环境反馈"的肌肉记忆；GSPO 相当于真正上路，在真实路口、人流、坡度中反复试错，根据每次是平稳通过还是差点摔跤调整转向和节奏。顺序不能反——不先看说明书直接上路，大概率连刹车在哪都不知道，第一天就摔惨。

Qwen-AgentWorld 还会挑出轨迹中真正包含环境变化的轮次重点训练。纯格式重复、没有新观察的轮次信息少，不必和关键操作获得相同权重。举个例子：agent 连续 5 次调用同一个 `search` 工具、输入完全相同的关键词，返回结果自然完全一样——第 1 次调用时模型真正读到了新搜索结果，这一步对学习"怎么根据搜索结果选下一步动作"有价值；第 2 到第 5 次反复调用同一个工具却没有新观察，本质是模型在"空转"，信息量几乎为零。给这些重复轮次和第 1 次同样权重，会稀释有效梯度，甚至让模型学会"重复调用工具凑长度"的坏习惯。这说明轨迹筛选已经从"整条保留或删除"深入到回合级别。

从闭环定位看，Qwen-AgentWorld 改了两步：一是 SFT 阶段不再直接教任务策略，拆成"继续预训练学环境建模 + SFT 学基本操作"两个子阶段，降低直接上手动作学习的门槛；二是 RL 阶段加入按轮次筛选有效轨迹的机制，把含有真实环境变化的回合作为重点训练样本。

### GLM-5.2

GLM-5.2 改进的是 RL 训练系统，解决同步 GRPO 在 agent 任务上因为长短轨迹耗时差异大导致 GPU 严重闲置的问题。工具任务时长差异很大，按组等待多个 rollout 会浪费算力，用具体数字算一下就清楚：假设有 32 张 GPU 并行做 rollout，同一个 batch 内 32 个任务时长分布是 8 个 30 秒、8 个 2 分钟、12 个 5 分钟、4 个 15 分钟。同步 GRPO 必须等最慢的 4 个任务全部跑完才能开始训练，也就是等 900 秒。在这 900 秒里，前 8 张 GPU 只花了 30 秒，闲置 870 秒；第二组 8 张花了 120 秒，闲置 780 秒；第三组 12 张花了 300 秒，闲置 600 秒。总闲置时间是 8×870 + 8×780 + 12×600 = 20400 秒——相当于 32 张卡里超过 22 张都在空转。

GLM-5.2 公开的 SAO（Single-policy Asynchronous Optimization）就是为了解决这个问题：每个提示只生成一条轨迹，做完一条就异步送入训练队列，不用等同组其他的；同时配套处理旧策略数据、价值估计和 token 裁剪。slime 则在工程上连接训练后端、SGLang rollout、自定义环境、验证器与数据缓冲区。

异步训练引入了一个同步训练没有的新问题：策略版本不一致。同步 GRPO 整个 batch 的轨迹都是同一版策略生成的，起跑线一样；但异步 SAO 里，队列中的轨迹可能来自 5 分钟前的策略，也可能来自 30 分钟前的旧策略，训练端此刻的模型参数已经更新了几十步。这就像一场考试，有的考生拿今年教材，有的拿去年旧教材，不能放在一起按同一标准打分。这就是"策略版本标记"的意义：每条轨迹必须携带生成它的策略版本号，训练侧拿到后先对比版本号——差得太远直接丢弃，差得不远就用重要性采样权重修正 loss，确保旧策略数据不会把新策略往错误方向带。

从闭环定位看，GLM-5.2 改了两步：一是 RL 阶段把同步组内 GRPO 改成异步单轨迹 SAO，通过策略版本标记和重要性采样解决数据不一致，大幅提升 rollout 侧 GPU 利用率；二是数据与环境阶段配套 slime 工程框架，把训练后端、rollout 引擎、自定义环境和验证器串成异步可扩展的生产流水线。
...
看完四个案例，读者可能会问：那么多团队的公开报告，我怎么快速知道它在改哪一环？下面给出快速定位索引，把各团队的贡献直接映射到对应的阶段。

下面保留各团队的完整公开实践，作为前面六个阶段的案例库。同一团队通常覆盖多个阶段，不适合强行拆散；查阅时可以从当前问题进入：

- 如果你在**目标定义**阶段遇到问题，优先看 OpenAI Operator/Codex、Apple、IBM Granite、xAI Grok 的公开报告。
- 如果你在**数据与环境**阶段遇到问题，优先看 MiniMax SWE Scaling、WebExplorer、UI-TARS、LongCat、Qwen-AgentWorld 的公开报告。
- 如果你在**SFT**阶段遇到问题，优先看 Qwen2.5、InstructGPT、Meta Llama、Microsoft Phi 的公开报告。
- 如果你在**RL/偏好优化**阶段遇到问题，优先看 DeepSeek、Qwen3、DAPO/VAPO、Kimi、GLM、Hunyuan 的公开报告。
- 如果你在**评测**阶段遇到问题，优先看 Claude 系统卡、Gemini、Llama 4、StepFun、多模态与安全案例的公开报告。
- 如果你在**数据回流**阶段遇到问题，优先看 Rejection Sampling、Long-to-Short、OPD、Expert Soup、Tulu 3 的公开报告。

这个索引可以这样用：第一部分是你当前卡住的阶段，第二部分是在对应阶段上有公开实践和具体经验的团队或技术方案。对照着看就知道，当你自己在闭环某一步遇到卡点时，可以直接去查对应的公开资料，它们里面一般都有具体的数据构造方法、奖励设计细节或工程踩坑记录。
...
走到这里，我们已经把工业后训练的全貌完整走了一遍。后训练最核心的思想不是某个特定算法，而是**闭环**——定义目标、构造数据与环境、SFT 冷启动、RL 提升、独立评测、失败回流，这六个阶段首尾相接，形成一个持续迭代的循环。为什么不能做成"SFT + RL 一次性做完就发布"的线性流程？因为第一轮训练永远只能覆盖团队已经想到的情况，模型一旦进入真实场景，必然会暴露出训练集里没有的边界失败：有的是沙箱环境不一致导致 reward 乱跳，有的是验证器被模型钻了空子，有的是模型根本不会用某种工具，有的是长任务前面全对但最后一步路径写错。独立评测把这些失败按原因分类，不是简单打个分就结束，而是把失败变成下一轮系统改进的输入——该补数据的补数据、该修环境的修环境、该升级 verifier 的升级 verifier、该加 SFT 轨迹的加轨迹，模型能力正是在这样一轮一轮的修正中逐步建立起来的。闭环的本质不是"多跑几轮训练"，而是建立一套能从失败中持续学习的系统。

六个阶段各自有明确的定位，缺一不可。**目标定义**回答"什么叫真正完成任务"，要写清成功标准、安全边界和成本约束，这一步模糊后面所有环节都会跟着模糊；**数据与环境**回答"怎么让任务可以重复、可以判分"，要把原始事件变成可启动、可回放、带验证器的训练样本，环境不一致一切训练信号都不可靠；**SFT 冷启动**教模型基本的行为格式和操作协议，让模型知道怎么用工具、怎么观察环境反馈、什么时候算任务完成，相当于教练先带学员练会基本动作；**RL/偏好优化**让模型通过反复试错提高成功率，在大量尝试中把高 reward 的行为概率推高，真正学会决策；**独立评测**回答"提升是不是真的"，必须在没见过的新任务、接近部署的配置下检验，防止模型过拟合训练环境或钻 verifier 空子；**失败回流**把评测发现的问题分类送回前面的环节，完成闭环的最后一环。六个阶段的产物也不只是模型权重——可复用的数据集、版本化的沙箱环境、逐步完善的验证器、能发现真实失败的评测体系，这些资产的长期价值往往超过单次训练得到的权重本身。

当任务从"输出文本答案"的数学题扩展到"在真实环境里做事"的代码修复、网页操作、桌面 GUI 等 Agent 任务时，六阶段闭环的每一步都长出了新的工程要求。数学 RL 的环境只需要题目文本和 20 行答案验证器，整个过程不需要启动外部进程、不需要持久化中间状态；但一个中型 Python 项目的代码修复沙箱要带 200MB 依赖、十几个启动命令、完整 git history，光拉起环境就要一分钟，episode 长度从百 token 级变到万 token 级，reward 延迟从"输出完立即判分"变到"几十步交互后才知道结果"，rollout 失败率从"最多答案格式错"变到"沙箱崩溃、依赖装不上、工具超时"都很常见。目标定义里要加工具权限、时间预算和安全边界，数据要携带环境快照和可恢复 checkpoint，SFT 要教基于环境观察的决策而不只是输出格式，RL 要处理异步经验和多轮轨迹奖励，评测要监控成本和安全回归，失败回流还要做细粒度的轨迹片段筛选和验证器版本更新。理解了这些差异，就不会天真地认为"数学 RL 跑通了，直接搬到 Agent 上就行"。

把二十余个团队的公开实践收束起来，可以看到四条反复出现的共同规律。第一，奖励从主观偏好分走向客观验证分、再走向真实环境执行分——reward 越接近真实世界的成功标准，长训练越稳定，能力迁移性越强；第二，数据从静态问答对变成可生成、可验证、可回放的完整交互环境——容器镜像、数据库快照、工具调用序列、环境观察日志都成为训练数据的一部分，数据工程复杂度远超"整理 JSONL"；第三，训练顺序从简单的"SFT→RL"两步走拆分成多阶段精细化流程——冷启动 SFT、reasoning RL、agentic RL、通用能力回流，有的团队还加上 continue pretraining 学环境、OPD 合专家，不同能力用不同数据、不同 reward、不同训练火候；第四，训练系统本身正在成为核心竞争力——从 DAPO/VAPO 的采样和 clip 工程补丁，到 GLM-5.2 的 SAO 异步、LongCat 的 DORA 流式 RL，工程系统决定了能不能训长任务、能不能扩到多卡、能不能千步稳定训练，很多时候不是算法不对，而是系统撑不到算法起效。

后训练闭环跑通以后，下一个问题是训练曲线怎么看、什么时候训练不稳定、哪些波动是正常的、reward 上升但评测不涨是哪里出了问题、entropy 塌缩了怎么救、verifier 被钻空子有什么早期信号。这些都是实际跑训练时每天都会遇到的问题，也是下一节要讨论的核心内容。[18.3 训练稳定性](./modern-industrial-practice) 将沿着这条闭环继续深入，带你认识训练过程中的关键监控指标，学会从 loss、reward、entropy、长度、格式错误率等曲线中判断模型是在真学习还是在过拟合，以及出现不稳定时应该从哪几个环节排查问题。
