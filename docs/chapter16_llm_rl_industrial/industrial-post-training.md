...
上一节我们把一次 RL 训练拆成了**生成、奖励、更新和参数同步**——这解决了"算法怎么在多卡上跑起来"的问题。但真正开始做工业训练时，你会遇到更大的问题：**第一批训练数据从哪里来？** 模型训完以后，怎么知道它是**真的学会了**，还是只是记住了训练集？那些做错的题目，怎么让它在下一轮训练中**不再犯**？

先看一个代码模型的真实例子。团队从真实 GitHub 仓库中收集 bug 报告，为每个问题准备可以运行的环境、**依赖锁定**和测试用例。模型先通过 **SFT** 学会读取仓库、调用工具和提交补丁，再通过 **RL** 反复尝试修复；测试程序给每次尝试打分。训练完成后，团队在**完全没见过的新仓库**上评测模型，把失败任务归类，补充新的数据和验证规则，然后开始下一轮训练。

这就形成了工业后训练的**基本闭环**——不是训完一轮就结束，而是**一轮接一轮地迭代**：
...
第一轮数据只能覆盖团队已经想到的问题。模型进入真实场景后，会暴露新的失败方式：代码能够编译但没有解决需求，工具调用格式正确但一直重复搜索，数学答案正确但推理过程碰巧猜中。评测发现这些问题后，团队需要补充任务、环境或奖励，再重新训练。

因此，工业后训练的产物不只有模型权重，还包括能够持续复用的数据集、环境、验证器和评测体系。模型能力正是在多轮闭环中逐步建立的。

## 一轮完整后训练的六个阶段

| 阶段        | 要回答的问题                           | 典型产物                         |
| ----------- | -------------------------------------- | -------------------------------- |
| 目标定义    | 模型最终要完成什么任务                 | 能力清单、成功标准与安全边界     |
| 数据与环境  | 怎样让任务可以训练、可以重复、可以判分 | 提示、轨迹、沙箱、测试与验证器   |
| SFT         | 模型是否会按正确格式开始行动           | 具备基本行为的冷启动模型         |
| RL/偏好优化 | 模型能否通过尝试提高成功率             | 针对目标能力优化后的模型         |
| 评测        | 提升是否来自真实能力，其他能力是否退化 | 独立指标、失败样本与回归测试     |
| 数据回流    | 下一轮应该补什么                       | 新任务、新轨迹和修改后的奖励规则 |

公开方案使用的算法名称很多，初学时先问它改变了表中的哪一个阶段。下面继续用代码修复任务走完六步，再把不同公司的方法放回对应位置。

**定义目标能力：先写清楚什么叫"修好"。** 假设团队要训练一个代码模型修复 Python 项目中的真实 bug。第一步先写清楚什么叫"修好"：新增测试必须通过，原有测试不能退化，模型不能修改无关文件，也不能绕过测试。由此得到能力清单、成功标准和安全边界。部署时如果还限制运行时间、工具次数和回答长度，这些约束也要在此时确定。

**构造数据与环境：让每个任务都能重复启动。** GitHub issue 只有问题描述，还不是可训练任务。团队要保存出错前的仓库版本，锁定依赖和启动命令，准备新增测试与回归测试，再把整个项目封装进可以重复启动的沙箱。同一个任务每次都从相同状态开始，模型得到的奖励才可以比较。

环境型任务还要覆盖执行方式的变化。MiniMax 会使用不同的 Agent Scaffold 采样同一类任务；Kimi K3 会改变工具、提示和上下文管理；Qwen-AgentWorld 同时使用容器、MCP、移动端和网页环境。这样可以减少模型只记住某一种外层模板的风险。

**SFT 冷启动：先教模型基本行为格式。** 模型一开始可能不会读取仓库、调用测试命令或提交补丁。团队先收集成功轨迹做 SFT，让模型学会工具格式、操作顺序和停止条件。SFT 在这里负责建立基本行为，还不要求模型通过反复试错找到更好的修复策略。

**RL 提升：通过反复试错提高成功率。** 具备基本行为以后，当前模型对同一个任务尝试多次。测试程序给每条轨迹计算奖励，PPO、GRPO 或偏好优化提高成功轨迹的概率。数学、代码和写作等能力也可以先训练成不同领域专家，再通过 On-Policy Distillation 合回一个模型；MiniCPM5、Kimi K3 和 NVIDIA Nemotron-Cascade 2 都公开了这条路线。

**独立评测：在没见过的任务上检验真实能力。** 训练结束后，团队在未参加训练的新仓库上检查补丁通过率、无效工具调用、平均成本和通用能力回归。评测配置要接近真实部署：长度预算、思考强度、量化方式、工具协议和上下文管理不同，都可能改变最终结果。训练奖励升高而独立评测不变时，模型可能只适应了训练环境或利用了验证器漏洞。

**失败回流：把问题送回下一轮训练。** 评测失败要按原因回流。任务覆盖不足就补充新问题；环境无法恢复就修复沙箱；测试过弱就升级验证器；模型不会某种工具操作就补成功轨迹；模型已经会操作但成功率仍低，再进入下一轮 RL。团队必须记录任务、环境、轨迹、奖励和模型版本，才能重放失败并判断应该修改哪一层。

这样看，六个阶段并非六套独立工作。前一阶段的产物是后一阶段的输入，最后的评测又会回到最前面的目标、数据和环境。[18.5 大规模 RL 数据工程](./data-engineering) 将继续展开其中的数据生产线。

## 环境型任务对闭环的扩展

数学 RL 的环境通常只有题目和答案验证器。代码仓库、网页和桌面应用把每个阶段都扩展了：目标中出现工具和成本约束，数据要携带环境，SFT 要教授操作协议，RL 要处理长轨迹，评测要复现部署条件，失败回流还要保存完整状态。

| 阶段        | 环境型任务增加的内容                     | 代表案例                        |
| ----------- | ---------------------------------------- | ------------------------------- |
| 目标定义    | 工具权限、时间预算、长度与安全边界       | Apple 端侧约束、OpenAI 工具任务 |
| 数据与环境  | 仓库、容器、网页状态、工具和可恢复检查点 | MiniMax M2.1、Kimi K3           |
| SFT         | 工具格式、环境观察、动作顺序和停止条件   | Qwen-AgentWorld、UI-TARS        |
| RL/偏好优化 | 多轮轨迹奖励、异步经验和领域专家训练     | GLM-5.2、DAPO、MiniCPM5 OPD     |
| 评测        | 真实环境、独立任务、成本与安全回归       | Claude 系统卡、Computer Use     |
| 数据回流    | 失败分类、拒绝采样、蒸馏与验证器更新     | DeepSeek-R1、Nemotron-Cascade 2 |

## 从四个公开案例看闭环的不同实现

下面四个案例分别改变了闭环中的不同位置。MiniCPM5 处理多个领域专家怎样合回一个模型；Kimi K3 把环境配置本身变成训练数据；Qwen-AgentWorld 在训练动作前先学习环境变化；GLM-5.2 则让长短不一的轨迹异步进入训练。阅读时先找它改变了六个阶段中的哪一步，再看使用了什么算法名称。

### MiniCPM5：领域专家 RL 与 OPD 能力合并

MiniCPM5-1B 的公开流程是 SFT、RL、OPD。团队先分别训练数学、代码、闭卷问答、写作等领域的 RL 教师模型，再让最终模型对同一批领域提示进行采样。教师模型对学生生成的每个位置给出概率分布，学生据此调整自己的输出分布。这就是 On-Policy Distillation（OPD）。

它和普通离线蒸馏的差别在数据来源。离线蒸馏先保存教师写好的答案，学生反复模仿这些固定答案；OPD 的回答由当前学生自己生成，因此教师纠正的是学生此刻真正会走到的状态。MiniCPM5 还复用了训练各领域教师时的提示，不需要再构造一套蒸馏题目。

从数据工程角度看，这套方法要求每条样本同时记录领域、提示版本、学生策略版本、教师版本和逐 token 的教师信号。OPD 的公式与日志结构放在 [18.5](./data-engineering) 的进阶部分。

### Kimi K3：可组合训练环境

Kimi K3 把工具、提示、上下文管理、技能、记忆和子智能体组合成统一训练环境。同一个任务可以换用不同工具配置或执行框架，避免模型只熟悉一种 harness。任务侧还可以从知识图谱选择相关节点，再检索公开文章、博客和代码仓库，合成长程研究任务。

Kimi K3 同样先训练不同领域和思考强度的专家，再用多教师 OPD 合并能力。对百万 token 的长任务，系统保存未完成轨迹和外部 KV 状态，下一轮从中间继续，而不必每次从头生成。

### Qwen-AgentWorld：环境建模与动作学习

Qwen-AgentWorld 使用容器沙箱、MCP 工具、Android、网页和操作系统模拟器构造任务。训练分为三个阶段：继续预训练让模型学习环境知识，SFT 让模型预测下一状态并学会基本操作，最后用 GSPO 和混合奖励训练完成任务。

它还会挑出轨迹中真正包含环境变化的轮次重点训练。纯格式重复、没有新观察的轮次提供的信息较少，不必和关键操作获得相同权重。这个例子说明，轨迹筛选已经从“整条保留或删除”深入到回合级别。

### GLM-5.2：异步单轨迹优化

工具任务的时长差异很大，按组等待多个 rollout 会浪费算力。GLM-5.2 公开的 SAO 让每个提示只生成一条轨迹，并持续异步送入训练；同时处理旧策略数据、价值估计和 token 裁剪。slime 则在工程上连接训练后端、SGLang rollout、自定义环境、验证器与数据缓冲区。

这说明数据队列并非简单缓存。异步训练中的每条轨迹必须携带生成它的策略版本，训练侧才能判断数据是否过旧、是否需要重要性修正或丢弃。
...
理解了六阶段闭环和四个代表性案例之后，实际动手训练时仍会遇到大量具体工程问题：冷启动数据如何构造、长度奖励在训练哪个阶段引入、异步采样如何保证数据版本一致性、多模态奖励如何加权平衡。这些问题在近两年的工业界技术报告中已有大量公开讨论。

本节整理国内外二十余个团队公开的后训练技术报告，按团队组织材料。读者不必顺序通读，可根据实际遇到的问题，对照前文的六阶段框架，查阅对应团队的具体做法。
...
MiniMax 的公开资料分三条线（[M2.1 Agent 后训练经验](https://www.minimax.io/news/post-training-experience-and-insights-for-agent-models)、[MiniMax-M1 长思考 RL](https://arxiv.org/abs/2506.13585)、[WebExplorer 网页 Agent](https://arxiv.org/abs/2509.06501)）：分别对应软件与应用 agent、长思考 RL、长程网页 agent，不应只概括成“可验证环境 + reward”。
...
DeepSeek 的公开材料（[DeepSeekMath](https://arxiv.org/abs/2402.03300)、[DeepSeek-R1](https://arxiv.org/abs/2501.12948)、[DeepSeek-V3.2](https://arxiv.org/abs/2512.02556)）是理解 GRPO / RLVR 的主线之一。DeepSeekMath 先给出 critic-free 的组内相对优势，R1 再证明纯规则 reward 可以诱发长思考，V3.2 则把可验证任务推进到 agentic task synthesis。
...
GLM 的主线是 ARC：Agentic、Reasoning、Coding（[GLM-4.5](https://arxiv.org/abs/2508.06471)、[GLM-5](https://arxiv.org/html/2602.15763v1)）。GLM-4.5 先证明这三类能力可以放在一个 MoE 模型里共同优化；GLM-5 则把后训练 recipe 讲得更清楚。
...
ERNIE 的公开资料（[ERNIE 4.5](https://ernie.baidu.com/blog/publication/ERNIE_Technical_Report.pdf)、[ERNIE 5.0](https://arxiv.org/abs/2602.04705)）披露很有结构：LLM post-training 是 SFT + RL，RL 阶段用 Progressive RL 和 Unified Preference Optimization；VLM post-training 则是三段 SFT + 一段 reasoning RL。ERNIE 的价值在于把多任务、多 reward、多模态后训练的“兼容性”问题讲得比较清楚。
...
StepFun 的公开资料（[Step3](https://stepfun.ai/research/en/step3)、[STEP3-VL-10B](https://huggingface.co/papers/2601.09668)、[Step-DeepResearch](https://arxiv.org/abs/2512.20491)）覆盖多模态 reasoning 和 deep research agent。STEP3-VL-10B 披露了一个紧凑 10B VLM 怎么靠 scaled post-training 追近更大模型；Step-DeepResearch 则属于研究型 agent 训练。
...
[LongCat-Flash-Thinking-2601](https://tech.meituan.com/2026/02/02/LongCat-Flash-Thinking-2601-techreport.html) 是一份很像“agent RL 工程系统设计说明”的报告。它的核心不是某个 reward 公式，而是环境扩展、强化学习扩展、噪声鲁棒训练和 heavy thinking。
...
Pangu 公开信息（[Pangu Ultra](https://github.com/pangu-tech/pangu-ultra)、[Pangu Pro MoE](https://arxiv.org/abs/2505.21411)、[盘古开源新闻](https://www.huawei.com/cn/news/2025/7/pangu-opensource)）的重点在昇腾原生训练、MoE 稀疏效率和开源模型体系，后训练细节没有像 R1/Qwen/MiniMax 那样展开。
...
[Yi-Lightning](https://arxiv.org/abs/2412.01253) 披露的是传统产品级 LLM 后训练线：pre-training 之后做 SFT 和 RLHF，并强调 multi-stage training、synthetic data construction、reward modeling，以及 RAISE 安全框架贯穿 pre-training、post-training 和 serving。它不像 agent 报告那样提供工具环境 recipe，但适合学习"聊天模型如何被人类偏好拉齐"。
...
[InternLM2](https://arxiv.org/abs/2403.17297) 是开源社区理解传统 RLHF 工程化的重要参考。它的重点不是长 CoT RLVR，而是数据治理、SFT、reward modeling 和 online RLHF。
...
两家公司的公开资料（[Baichuan 2](https://arxiv.org/abs/2309.10305)、[360Zhinao](https://arxiv.org/abs/2405.13386)）代表了经典的中文开源对齐路线。
...
Skywork-OR1 和 MiMo（[Skywork-OR1](https://huggingface.co/papers/2505.22312)、[MiMo](https://arxiv.org/abs/2505.07608)、[MiMo-VL-Miloco](https://arxiv.org/abs/2512.17436)）都适合学习"小模型 / 蒸馏模型继续做 RL"的问题。它们不像 frontier lab 只堆规模，而是关注 entropy collapse、数据难度、reward 稀疏和训练稳定性。
...
这三家公司公开了多模态后训练（快手 [Kwai Keye-VL](https://arxiv.org/abs/2507.01949)）、原生理解生成（商汤 [SenseNova U1](https://www.sensetime.com/en/news/51170629)）和深度推理（讯飞 [Spark X1](https://news.cgtn.com/news/2025-01-15/China-releases-Spark-X1-deep-reasoning-model-that-packs-a-punch-1AbIq8PzzEI/index.html)）的动态，但缺乏完整 recipe。阅读时看它们分别押注哪些能力面——VLM、多模态生成、深度推理、中文场景、端到端产品体验——不要从发布材料反推出未披露的 SFT/RL 细节。
...
Google DeepMind 的公开资料（[Gemini 1.5](https://arxiv.org/abs/2403.05530)、[Gemini 2.5](https://arxiv.org/abs/2507.06261)、[Gemini 2.5 Deep Think](https://blog.google/products/gemini/gemini-2-5-deep-think)、[Gemini 2.5 Computer Use](https://blog.google/innovation-and-ai/models-and-research/google-deepmind/gemini-computer-use-model/)、[Gemini 3.1 Pro](https://deepmind.google/models/model-cards/gemini-3-1-pro/)、[Gemma 3](https://arxiv.org/abs/2503.19786)）披露粒度不如开放论文，但方向非常明确：多模态、长上下文、工具、reasoning 和安全评测一起做。Gemini / Gemma 这条线适合学习“统一多模态模型怎么设计后训练任务”。
...
[The Llama 3 Herd of Models](https://arxiv.org/abs/2407.21783) 是开放模型里最适合作为产品级 chat model 后训练流水线的参考之一。它的价值不是某个单独算法，而是完整的数据治理、SFT、reward model、rejection sampling、preference optimization、安全对齐和评测闭环。
...
Phi 的公开资料（[Phi-4](https://arxiv.org/abs/2412.08905)、[Phi-4-reasoning](https://arxiv.org/abs/2504.21318)）重点是小模型 reasoning。[Phi-4-reasoning](https://arxiv.org/abs/2504.21318) 不是靠巨大参数硬推，而是靠高质量合成数据、teachable prompts 和一段 outcome-based RL，把 14B 级模型推到较强推理水平。
...
[Magistral](https://arxiv.org/abs/2506.10910) 的公开摘要很值得放在 reasoning RL 章节：Mistral 明确说它使用自家的 scalable RL pipeline，不依赖已有实现或从其他模型蒸馏来的 RL traces，而是 ground-up 做 pure RL。
...
**Apple Foundation Models**（[技术报告](https://machinelearning.apple.com/research/apple-foundation-models-tech-report-2025)）回答端侧模型的后训练如何与部署接口联合设计：guided generation、constrained tool calling 和 LoRA 支持被纳入训练目标，奖励拆成偏好 RM、规则 verifier 和 tool schema checker 三类混合，并按端侧/云端目标分别加权；端侧评测还要加入简洁性、延迟、内存和能耗。复现时可以让小模型按 JSON schema 训练工具调用，用 constrained decoder 评测 schema success rate。

**xAI Grok 与 Pi**（[Grok](https://x.ai/news/grok-4-1/)、[Pi](https://pi.ai/)）都把产品人格变成可评测的多目标奖励，而不是依赖 system prompt：personality、truthfulness、emotional intelligence、safety 四个目标分开训练 reward head，再做 multi-objective DPO/RL。人格评估不能只看单一分数，尤其要单独测 sycophancy，即模型是否为了讨好用户而承认错误事实。复现时构造多维 judge rubric，用偏好数据训练 multi-objective DPO/RL。

**IBM Granite**（[Granite 3.3/4.0/4.1](https://www.ibm.com/new/announcements/ibm-granite-4-0-hyper-efficient-high-performance-hybrid-models)）说明 GRPO/TPO 这类 reasoning RL 可以下沉到企业级小模型，并支持切换 thinking 模式。评测要同时覆盖 RAG 引用忠实性、函数调用成功率、拒答正确性和 thinking on/off 的差异。复现路线是 SFT→GRPO 强化可验证推理→偏好优化忠实性→model merging 融合领域专家。

**Salesforce xLAM / SFR-RL**（[博客](https://www.salesforce.com/blog/large-action-model-ai-agent/)）把工具调用奖励拆成 schema validity、tool selection accuracy、argument exact match、execution success、groundedness 五段，并用 Pipelined Synchronous RL 让 rollout 与 training 两阶段共享 GPU 集群交替执行。Inference gateway 负责自动检测 crash、重建 engine、恢复权重并重排 in-flight work，工具执行尽量本地化。复现时准备 50 个 mock API 生成请求和调用链，用函数执行结果做 verifier，并实现 timeout/retry/失败标记，避免系统错误污染数据。
...
**AI2 Tulu 3**（[论文](https://openreview.net/forum?id=i1uGbfHHpH)）的价值在透明度而非 SOTA：数据、代码、recipe 和评测全公开，覆盖 SFT→偏好学习→RLVR 三阶段。它清楚公开 prompt 混合方式、偏好构造、可验证奖励接入、超参数和评测组织，最适合作为后训练复现的第一站：先跑通全开源流程，再逐个加入 DAPO/VAPO/MiniMax 多 scaffold、Qwen 分阶段训练等技巧做 ablation，而不是一开始就堆砌改进。
