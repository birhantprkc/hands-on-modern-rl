...
> **本节目标**：理解稀疏奖励的归因困难，掌握 ORM 与 PRM 的数学形式，判断什么时候值得付出额外成本去训练过程奖励模型。
...
| 步骤                          | 距结尾步数 | 回报权重 $\gamma^{T-t}$ |
| ----------------------------- | ---------- | ----------------------- |
| Step 1（假设 $\sqrt{2}=p/q$） | 8 步       | $0.9^8 \approx 0.43$    |
| Step 5（设 $p=2k$）           | 4 步       | $0.9^4 \approx 0.66$    |
| Step 6（出错的地方）          | 3 步       | $0.9^3 \approx 0.73$    |
| Step 8（推出矛盾）            | 1 步       | $0.9^1 \approx 0.90$    |
...
| 任务情形                  | 只用 ORM 会怎样                                      | PRM 可能增加的价值                       |
| ------------------------- | ---------------------------------------------------- | ---------------------------------------- |
| 几百 token 以内的短回答   | 错误位置离结尾近，重新采样成本低                     | 通常有限                                 |
| 包含多步变换的证明或程序  | 最终失败时不知道哪一步该保留、哪一步该改             | 定位首个错误，保留正确前缀，不用从头再来 |
| 数千 token 的工具调用轨迹 | 早期错误会触发一连串无效工具调用，浪费大量时间和费用 | 及早停止、回退到正确节点、换一条路走     |
