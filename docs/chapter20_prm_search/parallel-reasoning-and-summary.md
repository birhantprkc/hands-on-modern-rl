...
> **本节目标**：理解串行深度与并行广度两种算力分配的适用条件，掌握 PaCoRe 的多轮协调结构、GenRM 的工作原理，并建立整章 verifier 与推理策略的组合决策线。
...
| -------- | --------------------------- | -------------------- | ---------------------------------------------- | ---------------------------------------------------- |
| 串行深度 | 长推理、顺序修订、树搜索    | 一条路径连续保留状态 | 后一步依赖前一步的证明、代码调试与工具调用任务 | 轮数增加会累积墙钟延迟（没法并行），早期错误也会传播 |
| 并行广度 | Best-of-N、Self-Consistency | 多条路径独立生成     | 存在多种独立解法且结果容易比较的任务           | 总计算随路径数线性增加，还需要一个可靠的聚合器       |
...
| 预算控制 | 每轮路径数和轮数       | 产品 effort 或模式选择               | 迭代数、扩展数与 rollout 长度 |
| 反馈     | 训练主要使用结果奖励   | 公开资料未披露完整结构               | 结果、PRM 或外部 verifier     |
| 主要代价 | 大量完整轨迹与消息压缩 | 难以复现内部协调机制                 | 串行选择、状态维护与多次评分  |
...
| 维度 | 判别式 RM                  | GenRM                                          |
| ---- | -------------------------- | ---------------------------------------------- |
| 输出 | 标量、类别或偏好概率       | 评价文字 + 判断 token                          |
| 训练 | 回归、分类或排序损失       | 下一个 token 预测，可直接配合偏好数据          |
| 调试 | 主要依赖分数分布和错误样本 | 可以直接阅读评价依据，但仍要验证评价本身对不对 |
| 推理 | 一次前向传播即可得到分数   | 需要生成若干评价 token，通常更慢               |
...
- **17.4 形式化 Verifier**：AlphaProof、Lean4 与 DeepSeek-Prover-V2，用形式化证明获得完全可靠的每步反馈
