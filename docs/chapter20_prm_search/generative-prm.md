...
> **本节目标**：理解生成式 PRM 的输出结构与数据构造方法，弄清"约 1% 的过程标签"的真实含义，掌握判别式与生成式 PRM 的选择标准。
...
| 维度         | 判别式 PRM               | 生成式 PRM                                     |
| ------------ | ------------------------ | ---------------------------------------------- |
| 输出         | 步骤类别或概率           | 检查过程文本 + 可抽取判断                      |
| 单次评价速度 | 一次前向传播，吞吐量高   | 需要自回归生成，延迟高                         |
| 数据需求     | 需要足够标签校准分类边界 | 仍需标签筛选生成评价，但微调样本少             |
| 领域迁移     | 取决于训练数据覆盖和基座 | 能调用基座文本知识，但也可能生成流畅的错误解释 |
| 增加验证计算 | 模型集成、多模型投票     | 增加生成长度、复核轮数、并行采样数             |
| 可检查性     | 只能看分数和误差样本     | 能直接阅读判断依据，但依据不一定忠实或正确     |
...
| 场景                | 判别式（一次前向） | 生成式（自回归检查）       |
| ------------------- | ------------------ | -------------------------- |
| RL 训练，每轮百万步 | 百万次前向，可批量 | 百万段生成，成本高一个量级 |
| 线上重排序 64 候选  | 完全可以接受       | 取决于延迟预算             |
| 调试 10 条失败样本  | 只能看到分数       | 逐条给出错误原因           |

### 混合两类 PRM

没有公开训练细节时，不要从产品输出反推内部是否用了生成式 PRM。从公开方法来看，至少三种组合方式是可行的：
