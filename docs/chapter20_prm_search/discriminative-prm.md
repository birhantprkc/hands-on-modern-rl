...
> **本节目标**：走完判别式 PRM 的完整数据生产线：步骤切分、标注、分类器训练、分数聚合，并理解它在重排序和 RL 训练中的两种用法与四个风险点。
...
| 模型给 good 的概率 | 损失 $-\log p$ | 含义                 |
| ------------------ | -------------- | -------------------- |
| $0.95$             | $\approx 0.05$ | 几乎不惩罚，判断很准 |
| $0.70$             | $\approx 0.36$ | 温和惩罚，还能更好   |
| $0.10$             | $\approx 2.30$ | 强烈惩罚，严重错判   |
...
| 候选 | 步骤分数                  | Mean   | Min   | Product |
| ---- | ------------------------- | ------ | ----- | ------- |
| 甲   | $[0.9,\ 0.8,\ 0.2,\ 0.9]$ | $0.70$ | $0.2$ | $0.130$ |
| 乙   | $[0.6,\ 0.6,\ 0.6,\ 0.6]$ | $0.60$ | $0.6$ | $0.130$ |
