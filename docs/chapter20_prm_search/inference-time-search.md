...
> **本节目标**：理解独立采样的两处浪费，掌握 Beam Search、Tree of Thoughts 和 MCTS 三种搜索结构的差别，会用 UCB 公式做具体计算，并判断什么时候值得付出搜索的算力开销。
...
| 轮次 | 候选（分数）                                                                  | 保留（$K=2$）                      |
| ---- | ----------------------------------------------------------------------------- | ---------------------------------- |
| 1    | 因式分解 $0.9$、求根公式 $0.85$、配方法 $0.4$                                 | 因式分解、求根公式                 |
| 2    | 因式分解→令因子为零 $0.92$、求根公式→算判别式出错 $0.3$、求根公式→算对 $0.88$ | 因式分解→令因子为零、求根公式→算对 |
...
| 子节点 | $Q(n)$ | $N(n)$ | 探索项 $c\sqrt{\ln N(p)/N(n)}$             | UCB    |
| ------ | ------ | ------ | ------------------------------------------ | ------ |
| A      | $0.8$  | $50$   | $1.414 \times \sqrt{4.61/50} \approx 0.43$ | $1.23$ |
| B      | $0.7$  | $30$   | $1.414 \times \sqrt{4.61/30} \approx 0.55$ | $1.25$ |
| C      | $0.6$  | $5$    | $1.414 \times \sqrt{4.61/5} \approx 1.36$  | $1.96$ |
