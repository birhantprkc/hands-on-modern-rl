...
> **本节目标**：理解形式检查的确定性从何而来，AlphaProof 与 DeepSeek-Prover-V2 怎样组织证明搜索，以及形式化方法的适用边界。
...
有没有一种检查器不会出错？对就是对，错就是错，没有"大概 80% 概率正确"这种说法？

有。如果题目本身已经被写成了 Lean4、Coq、Isabelle 这类形式语言，可以把候选证明交给**证明检查器**。检查器不关心证明"看起来合不合理"，它只做一件事：按照形式系统的规则，逐步验证每个证明项是否符合要求。通过就是通过，不通过就是不通过，反馈是完全确定的。
...
候选证明是一串 tactic 组成的 Lean4 代码，内核检查就是编译这段代码，验证每个 tactic 产生的证明项是否合法。$\mathbb{1}[\cdot]$ 是指示函数：检查通过取 1，任何一步失败取 0。和 17.2 的步骤分类不同，这个奖励没有标注噪声，错误只可能来自题目翻译和证明搜索，不会来自判分本身。但有两个严格前提：自然语言题目被忠实地翻译成了形式命题；任务本身是可以形式化的。检查器只验证"翻译后的命题被证明了"，不验证翻译本身对不对。
...
- **完全确定性**：在给定形式系统、公理、实现正确的前提下，通过就是通过，不通过就是不通过，同一个证明跑 100 次结果都一样，没有随机因素
...
形式化过程反馈用 Lean4 这类证明检查器替代神经网络判断：候选证明必须构造出目标命题的证明项，内核按规则逐步检查，反馈是完全确定性的。AlphaProof 和 DeepSeek-Prover-V2 都证明了：语言模型负责提出候选和搜索方向，证明检查器负责验证规则，这个组合可以解决相当难的数学竞赛题。
