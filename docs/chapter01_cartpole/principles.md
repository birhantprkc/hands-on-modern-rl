...
一根杆子通过转轴立在小车上，控制器每一步只能在"向左推"和"向右推"里二选一。

训练开始时，它的决策近乎随机，杆子几步就倒。每存活一步，环境给 1 分，策略就朝着得分更多的方向调整。训练几十轮后，同一套代码能稳定撑满 500 步——没有任何人写下过"杆子向左倾就向右推"这样的规则，平衡完全是试错换来的。

这就是强化学习要解决的问题：让智能体通过与环境的反复交互，自己学会每一步的更好选择。

本节就用 CartPole 完成第一个实验。这个任务小到普通笔记本 CPU 半分钟就能训练完，又完整到观测、动作、奖励、策略和价值估计都有各自的位置。

本节先跑通一次训练，看奖励曲线从 20 分附近爬向 500 分；再拆开环境，弄清观测、动作和奖励分别是什么；然后引入策略网络和价值网络，看二者怎样配合；最后把一次完整的 PPO 训练流程连起来。
