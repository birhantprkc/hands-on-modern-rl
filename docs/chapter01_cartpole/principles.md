...
奖励曲线从 20 分涨到 500 分，变化发生在每一步的交互里。要看清这一点，先得理解环境本身的规则。
...
图 1-2 来自环境的实际渲染。该回合撑满 500 步上限，说明训练后的策略能够持续控制小车，而不只是在某一帧短暂保持直立。

要看懂这个环境，需要回答三个问题：智能体能看到什么（观测）、能做什么（动作）、怎样判断好坏（奖励）。
...
有了优势，还需要一个更新规则来调整策略。这里有一个问题：同一批数据来自更新前的旧策略。如果新策略可以随意偏离旧策略，一次更新就可能把策略推到一个很差的区域。
