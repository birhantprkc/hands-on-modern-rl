...
前一章我们用 PPO 解决了 Actor-Critic 的训练稳定性问题——裁剪目标约束了每次更新的幅度，让训练不再轻易崩溃。但 PPO 留下了另一个问题：它是 **on-policy** 的——每次更新参数后，之前采集的数据就不能再用了，必须重新用新策略采样。在仿真器里这可能不算什么（百万步几小时就能跑完），但在真实世界里——比如机械臂、自动驾驶、机器人控制——每一步真实交互都有成本，on-policy 的低样本效率是致命的。

能不能把 DQN 的 off-policy 优势（经验回放、复用历史数据）"搬"到连续动作上来？DDPG 给出了第一个答案。
