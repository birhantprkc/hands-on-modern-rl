...
> **本节目标**：理解同一组参数怎样同时支持长推理和直接回答两种模式，thinking budget 怎样控制思考长度，以及在相同预算下"一条长链"和"多条短回答"应该怎样选择。
...
2025 年 8 月发布的 [DeepSeek V3.1](https://api-docs.deepseek.com/news/news250821) 是最早把这种双模式做成公开产品的例子。它在同一个模型版本上暴露了两个 API 端点：
...
| 任务特征               | 更适合 Thinking（一条长链） | 更适合 NoThinking（多条短回答） |
| ---------------------- | --------------------------- | ------------------------------- |
| 是否需要连续推导依赖？ | 是（证明、代码、多步规划）  | 否（问答、选择题、简单计算）    |
| 单次短回答成功率？     | 低                          | 中高                            |
| 有没有可靠验证器？     | 没有也能工作                | 最好有                          |
...
| 方法                        | 作用层面        | 核心思路                                           |
| --------------------------- | --------------- | -------------------------------------------------- |
| 模型权重合并                | 参数层面        | 直接平均长 CoT 模型和短 CoT 模型的权重             |
| Shortest Rejection Sampling | 监督数据（SFT） | 从多条正确回答中选最短的那条做监督微调             |
| DPO                         | 偏好数据        | 把"短而正确"作为 chosen，"更长或错误"作为 rejected |
| long2short RL               | RL 奖励         | 在 RL 中加入组内长度奖励，同时缩短 rollout 上限    |
...
| 回答 | $\operatorname{len}(i)$ | $\dfrac{\operatorname{len}(i)-l_{\min}}{l_{\max}-l_{\min}}$ | $\lambda_i$（长度奖励） |
| :--: | :---------------------: | :---------------------------------------------------------: | :---------------------: |
|  甲  |          $800$          |                             $0$                             |          $0.5$          |
|  乙  |         $1200$          |                   $400/1200 \approx 0.33$                   |     $\approx 0.17$      |
|  丙  |         $2000$          |                             $1$                             |         $-0.5$          |
