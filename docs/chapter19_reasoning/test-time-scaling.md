...
> **本节目标**：区分知识型失败与采样型失败，掌握并行采样、顺序修订和树搜索三种推理计算的用法，并判断什么时候应该停止继续加计算。
...
| 失败类型       | 原因                                                         | 解决方法                                         |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------ |
| **知识型失败** | 模型根本没掌握解题所需的知识或方法                           | 只能靠继续训练（更多数据、更大模型）解决         |
| **采样型失败** | 模型掌握了方法，但这次生成时选错了路线、算错了数字、或没检查 | 可以靠增加推理计算（多采样、多修订、多搜索）补救 |
...
| 错误位置         | 并行采样表现               | 顺序修订表现                     |
| ---------------- | -------------------------- | -------------------------------- |
| 最后一步符号错误 | 浪费：连正确步骤一起重生成 | 高效：只改最后一步               |
| 第一步方法选错   | 高效：换一条路就可能走对   | 低效：在错误前缀上修补，越走越远 |
