...
> **本节目标**：分清内部推理、执行轨迹和用户解释三个对象，理解隐藏和展示推理链各自的理由，以及为什么"可见"不等于"忠实"。
...
| 对象                                    | 包含什么内容                                             | 谁可以访问                     | 主要用途                                   |
| --------------------------------------- | -------------------------------------------------------- | ------------------------------ | ------------------------------------------ |
| **内部推理（Internal Reasoning）**      | 未完成的假设、被放弃的路径、试错过程、重复计算、自我怀疑 | 模型本身、服务提供方的内部系统 | 供模型继续生成下一步 token，内部监控和调试 |
| **执行轨迹（Execution Trace）**         | 推理文本 + 工具调用记录与返回 + 时间戳 + 中间评分        | 系统日志、审计系统             | 事后审计、问题回放、失败分析、模型改进     |
| **用户解释（User-Facing Explanation）** | 整理后的关键依据、引用来源、计算过程、最终结论           | 最终用户                       | 用户阅读、核查结果、教学场景               |
...
| 输出策略             | 用户看到什么                        | 主要收益                                     | 仍需解决的问题                                                   |
| -------------------- | ----------------------------------- | -------------------------------------------- | ---------------------------------------------------------------- |
| **完全隐藏原始推理** | 只有最终答案                        | 输出最简洁，内部轨迹不直接暴露，安全处理简单 | 外部用户无法检查中间过程，只能靠服务方审计                       |
| **结构化推理摘要**   | 关键依据、引用、关键步骤 + 最终答案 | 兼顾可读性和部分检查能力，阅读成本低         | 摘要可能遗漏关键步骤或改写原意，需要验证摘要和答案的一致性       |
| **完整可见推理**     | 模型生成的全部推理文本 + 答案       | 最便于复现、教学、错误分析和研究             | 内容需要安全过滤，可能包含敏感信息或被放弃的错误路径，阅读成本高 |
...
| 目标             | 隐藏原始推理                         | 展示推理文本                                     |
| ---------------- | ------------------------------------ | ------------------------------------------------ |
| **训练资产保护** | 轨迹较难被直接大规模收集             | 轨迹更容易被复用和蒸馏                           |
| **外部可检查性** | 依赖服务方提供的摘要、引用、日志     | 用户可以直接检查文本步骤，但仍需验证其忠实性     |
| **内容安全**     | 返回前可以统一做安全处理和脱敏       | 展示内容本身也要接受和最终答案同等级别的安全过滤 |
| **教学与复现**   | 需要单独生成适合教学的解释材料       | 更方便学习者观察方法、分析错误样例               |
| **内部监控能力** | 服务方自己有完整轨迹，可以做内部监控 | 公开文本只覆盖可见部分，隐藏激活仍需其他方法监控 |
