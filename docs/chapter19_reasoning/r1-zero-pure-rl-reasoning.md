...
> **本节目标**：理解 R1-Zero 纯 RL 训练的起点条件，弄清 GRPO 怎样利用结果奖励产生学习信号、未被直接奖励的长推理行为为什么会出现，以及纯 RL 为什么仍需要冷启动与后续对齐。
...
DeepSeek-R1-Zero 在 2025 年 1 月用一次公开实验回答了这个问题。他们直接从预训练基座模型出发，不经过任何人工思维链的 SFT 阶段，只用正确性奖励和格式奖励做 GRPO 训练，结果模型自发学会了生成长思维链、进行反思和自我验证。这就是著名的"**Aha Moment**"。
...
| 单题答对概率 $p$ | 全对或全错的概率 $p^8+(1-p)^8$ | 组内学习信号 |
| ---------------- | ------------------------------ | ------------ |
| $0.9$（简单题）  | 约 $0.43$                      | 经常消失     |
| $0.7$            | 约 $0.06$                      | 大部分时候有 |
| $0.5$（中等题）  | 约 $0.008$                     | 几乎总有     |
| $0.3$            | 约 $0.06$                      | 大部分时候有 |
| $0.1$（难题）    | 约 $0.43$                      | 经常消失     |
...
| 行为模式         | 生成概率 | 最终正确率 | 对总奖励的贡献      |
| ---------------- | -------- | ---------- | ------------------- |
| 短：直接猜答案   | $0.7$    | $0.1$      | $0.7\times0.1=0.07$ |
| 长：推导后再验算 | $0.3$    | $0.6$      | $0.3\times0.6=0.18$ |
