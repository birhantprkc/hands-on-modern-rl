...
> **本节目标**：理解普通语言模型串行生成的局限，弄清 o1 在产品与研究层面带来的行为变化、强化学习在其中的实际作用，以及系统如何区分思考内容与最终答案。

前面的章节从 PPO、DPO 到 GRPO，我们一直在讨论如何用强化学习让语言模型更符合人类偏好、在数学和代码任务上拿到更高的分数。但在 GRPO 训练中你可能已经观察到一个现象：模型面对一道稍有难度的题时，往往读完题目就直接往下写答案。如果第一步公式选错了，后面的几百个 token 都只是在这条错误路径上继续。
...
代码任务里同样如此。模型可以一次写出一个完整补丁，却不会先运行测试看看问题在哪、再根据报错修改方案。让你一次性写出没有 bug 的几百行代码，你也很难做到。但人会编译、跑测试、看报错、定位问题、局部修改，而 LLM 的一次连续生成缺少的正是这个"检查—修正"的循环。

## o1 把思考做成产品接口

2024 年 9 月，OpenAI 发布 o1，标志着推理模型从研究概念变成了可调用的产品。o1 的核心变化不在于"模型更大"，而在于**模型在给答案之前可以先思考**，并且思考的长度可以作为一个参数调节。
...
| 基准         | 评测方式  | GPT-4o | o1     |
| ------------ | --------- | ------ | ------ |
| AIME 2024    | 单次回答  | 约 13% | 约 74% |
| AIME 2024    | 64 次共识 | 约 56% | 约 83% |
| Codeforces   | Elo 等级  | —      | 1673   |
| GPQA Diamond | 单次回答  | 约 51% | 约 78% |

其中 Codeforces 的 Elo 为 1673，位于参评人类选手的第 89 百分位。

这张表里最容易误读的是第二行。GPT-4o 单次只有约 13%，采满 64 条回答做多数选择后可以达到约 56%。采样确实能提高分数，但这是**评测方式带来的提升**。o1 单次就达到约 74%，已经高于 GPT-4o 采样 64 次的水平；再给 o1 64 次共识，又到约 83%。单次回答、多数选择和重排序使用的推理计算不同，把指标写在一起时必须保留评测方式，否则读者会把"模型更强"和"采样更多"混为一谈。

用一个具体数字感受这个差距：如果模型单次正确率是 $p=0.13$，采 64 次至少有一条正确的概率是 $1-(1-0.13)^{64}\approx 99.9\%$。但"能覆盖到正确答案"和"能把正确答案选出来"是两回事，多数投票在难题上的实际提升受限于不同路径是否会犯系统性错误。o1 单次 74% 的含义是：模型自己的一次内部思考，就已经完成了大部分"尝试—检查—修正"的工作。
...
| `reasoning_effort` | 思考长度 | 单题延迟 | 适用请求                       |
| ------------------ | -------- | -------- | ------------------------------ |
| 低                 | 短       | 低       | 翻译、改写、简单问答、格式转换 |
| 中                 | 中       | 中       | 常规代码编写、中等难度数学题   |
| 高                 | 长       | 高       | 竞赛题、多约束规划、复杂调试   |
...
R1-Zero 没有见过任何人工标注的 CoT 数据，但训练后会自主生成长思维链、做反思、做验证。这种"涌现"是相对于训练数据而言的：模型没有被教过"你应该先检查再继续"这样的规则。
...
用一个类比理解这个区别：一个学生可能在潜意识里知道"做完题应该验算"，但考试时因为赶时间从来不做。你不需要从零教他什么是验算，只需要改变激励：让验算过的卷子得分更高，他就会逐渐把验算变成稳定的习惯。RL 在这里起的是类似"习惯养成"的作用，而不是"从零教学"。
...
| 结果类型           | 出现次数 | 特征                     |
| ------------------ | -------- | ------------------------ |
| 直接给出错误答案   | 72 次    | 没有中间步骤，直接猜结果 |
| 有推理但计算错误   | 22 次    | 列了公式但代数运算出错   |
| 推理完整且答案正确 | 6 次     | 包含检查和验证，结果正确 |
