...
> **本节目标**：理解模型怎样根据任务难度和当前进展动态决定投入多少计算，成本敏感奖励怎样同时激励"答对"和"省计算"，以及自适应预算带来的成本、延迟和安全问题。
...
|                  | 固定 Thinking Budget             | Adaptive Thinking                        |
| ---------------- | -------------------------------- | ---------------------------------------- |
| **谁决定用量**   | 调用方在请求时预设一个固定上限   | 模型根据当前请求动态决定                 |
| **成本可预测性** | 好，单次请求最多花多少是确定的   | 差，不同请求的用量可能差很多             |
| **灵活性**       | 差，简单题浪费预算，难题可能不够 | 好，能跳过简单题，也能延长复杂任务       |
| **适合场景**     | 批量任务、成本敏感的高并发服务   | 研究、代码、长工具调用等难度差异大的场景 |
...
#### 训练信号：让"答对"和"少算"同时进入奖励

要让模型学会按难度分配计算，训练奖励里必须同时包含"答案质量"和"计算成本"两项。成本敏感奖励公式为
...
这个式子没有规定具体用什么 RL 算法，它只说明两种压力怎样同时进入目标：
...
| 请求        | 方案     | $r_{\text{task}}$（对错） | $C$（token） | 成本惩罚 $\lambda C$ | $r_{\text{total}}$（净奖励） |
| ----------- | -------- | ------------------------- | ------------ | -------------------- | ---------------------------- |
| 翻译"hello" | 直接回答 | 1（答对）                 | 200          | 0.04                 | 1 - 0.04 = **0.96**          |
| 翻译"hello" | 长思考   | 1（答对）                 | 3000         | 0.6                  | 1 - 0.6 = **0.4**            |
| 竞赛数学题  | 直接回答 | 0（答错）                 | 200          | 0.04                 | 0 - 0.04 = **-0.04**         |
| 竞赛数学题  | 长思考   | 1（答对）                 | 3000         | 0.6                  | 1 - 0.6 = **0.4**            |
...
| 维度         | 固定预算                      | Hybrid Thinking（双模式）          | 自适应思考                                         |
| ------------ | ----------------------------- | ---------------------------------- | -------------------------------------------------- |
| **控制方式** | 每次请求用预设的固定 token 数 | 在"直接回答"和"思考模式"之间二选一 | 模型在 effort 和硬上限内自己决定是否继续、继续多久 |
| **优点**     | 延迟和成本最容易预估          | 简单题可以跳过长思考，体验好       | 能随任务进展动态调整，难题给够、简单题不浪费       |
| **风险**     | 简单题浪费、难题可能不够      | 路由器选错模式会造成损失           | 成本波动大，停止条件和安全更难评测                 |
| **适合场景** | 难度分布非常稳定的批量任务    | 通用对话和推理混合服务             | 代码、研究、长工具调用等难度差异大的任务           |
