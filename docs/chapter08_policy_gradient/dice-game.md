...
> **学习路径**：[2.1 探索与利用](../chapter03_mdp/bandit) → [6.1 策略梯度定理](./policy-gradient) → **6.4 摇骰子赌博机**
...
> 第 8 章 [PPO](../chapter10_ppo/intro) 中的 entropy bonus（熵奖励）就是强制策略保持探索的机制——它在损失函数里加了一个惩罚项，防止策略过早变得"太确定"。
...
本节从一次实际更新观察了 `-log_prob * reward` 怎样改变动作概率。[策略梯度定理](./policy-gradient)将进一步说明这个更新式从哪里来。
