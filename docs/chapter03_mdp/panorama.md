...
第 2 至 4 章围绕序列决策建模、回报定义、价值函数、贝尔曼方程、价值估计、Q-Learning、策略优化和奖励设计展开。本页是这一组基础章节的总结：先概括主要结论，再给出核心公式的矩阵形式与依赖关系，最后用一组复习问题帮助自测。
...
第 2 至 4 章的公式均采用逐状态（标量）形式。将所有状态排成向量、转移关系写成矩阵后，$n$ 个标量方程可压缩为一行矩阵方程。
...
下一章进入[第 3 章：价值函数与贝尔曼方程](./value-bellman)，正式推导价值函数的递归结构。
