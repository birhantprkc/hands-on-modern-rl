...
这个式子是什么意思？我们把它拆开看：智能体在状态 $s$，选择了动作 $a$，环境因此转移到了某个下一状态 $s'$——发生完这一整件事之后，环境给智能体一个数字，这个数字就是 $R$。它就是一个普通的实数，一个标量。
...
这也解释了为什么奖励设计会决定行为。算法看到的不是你脑子里的想法——不是"稳稳立住杆子"这个画面，不是"走到迷宫出口"这个中文指令，不是"给用户一个有帮助的回答"这个意图——它看到的是一串数字，然后努力让这些数字的期望和最大。如果这串数字和你真正想要的东西之间有偏差，会发生什么？算法越强，它就越快地把这个偏差利用到极致。
...
现在你看清楚区别了吗？$R$ 是**环境直接给的**——它是任务定义的一部分，是你写的规则。$V^\pi$ 不是环境给的——它是**算出来的**，是在某个策略 $\pi$ 下，从这个状态出发未来能拿到多少总回报的估计。价值永远依附于奖励而存在。如果我们把每一步奖励从 -1 改成 +1，整个价值函数都会翻转，学到的策略也会完全反过来。价值没有独立于奖励的意义——它永远是"在某个奖励定义下"的价值。
...
**量纲不匹配**。$r_{\text{forward}}$ 可能在 0~5，$r_{\text{ctrl}}$ 可能在 -100~0。数值大的项会主导梯度方向。这不是"哪个更重要"的权重问题，是纯粹的数值尺度问题。简单的缓解办法是手动把每个子奖励缩放到相近范围（比如 [-1, 1]）。
...
下一节我们会把本章讲过的 MDP、回报、价值函数、贝尔曼方程、算法分类路线和奖励设计，全部放回同一张全景图里。
