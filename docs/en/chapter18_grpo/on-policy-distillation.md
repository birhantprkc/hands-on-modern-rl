...
Consider an example with only three candidate tokens. After reading “`x + 3 = 7`, so `x =`,” the model must score the next token. Suppose it assigns the following raw scores to `4`, `5`, and a newline:
...
The first row contains **logits**. They are relative scores emitted directly by the model. They can be any real numbers and are not yet probabilities. Softmax converts the logits over the full vocabulary into a probability distribution that sums to 1:

$$
p(v)=\operatorname{softmax}(z)_v
$$

Here $z$ denotes the logits at this position, and $v$ denotes one candidate token in the vocabulary. In the example, token `4` receives probability about 0.67.

Taking the natural logarithm of a probability gives its **log probability**, abbreviated as `logp` in code. Log probabilities across multiple positions are stored in `logps`. The transformation is:
...
Probabilities do not exceed 1, so log-probabilities are usually at most 0. A value closer to 0 means the model assigns that token more probability: `-0.41` corresponds to a larger probability than `-2.41`.

Log-probabilities also simplify computation. The joint probability of a response multiplies its token probabilities; after taking logarithms, that product becomes a sum. The teacher-student log-probability difference can also be written as a probability ratio:
...
A positive value means the teacher assigns more probability than the student to the generated token $y_t$. A negative value means the teacher assigns less.

### What Is `full_ids`?

A language model receives numbers. The tokenizer first splits text into smaller tokens, then looks up a numeric identifier for every token. This identifier is called a token ID.

Consider one simple question. To make the process visible, suppose the tokenizer splits the question and response into five tokens. The IDs in the table are illustrative; they are not fixed IDs from a real tokenizer.
...
Joining the prompt and model response produces the complete sequence:
...
In the variable name, `full` means “prompt + complete response,” and `ids` means the numeric identifiers of those tokens. A batch is a group of samples processed together; the two outer brackets mean that this batch contains one complete sequence. The first three IDs belong to the prompt, so `prompt_len = 3`; the last two belong to the response generated by the student.
...
- `torch.log_softmax()` performs softmax and the logarithm together, directly producing the log-probability of every candidate token in the vocabulary.
- `gather()` uses `next_ids` to select the next token that actually appears in the sequence. If the three-candidate example generates `4`, it keeps `-0.41` and discards the scores for `5` and the newline.
- `next_token_logps()` returns a tensor with shape `[batch_size, sequence_length - 1]`. The same function is used for student and teacher, and GRPO and other RL algorithms reuse this log-prob computation.
...
Continue with “`2+2 answer is 4.`” The prompt contains three tokens and the response contains two. Aligning the original sequence with `next_token_logps` by prediction target reveals a one-position shift:

| Original position              | 0      | 1        | 2       | 3        | 4        |
| ------------------------------ | ------ | -------- | ------- | -------- | -------- |
| Text token                     | `2+2`  | `answer` | `is`    | **`4`**  | **`.`**  |
| `full_ids`                     | 18     | 42       | 9       | **27**   | **6**    |
| Token type                     | prompt | prompt   | prompt  | response | response |
| Corresponding `logps` position | —      | 0        | 1       | **2**    | **3**    |
| Token predicted there          | —      | `answer` | `is`    | **`4`**  | **`.`**  |
| Reward slice                   | —      | discard  | discard | **keep** | **keep** |

The first response token `4` has ID 27 and is stored at `full_ids[0, 3]`. Python counts from 0: the first index `0` means the first sample in the batch, and the second index `3` means the fourth token in that sample.

The model predicts the next token after reading `is` at position 2, so the score for `4` is stored in `logps[:, 2]`. Here `:` means every sample in the batch, while `2` means the third log-prob position. Because `prompt_len = 3`, response log-probabilities begin at `prompt_len - 1 = 2`.

Here reward can first be understood as the teacher's score for each token. A positive value increases the student's tendency to generate that token in the future, while a negative value decreases it.

We can now explain why rewards are marked “discard” or “keep.” The dataset supplies the prompt “`2+2 answer is`”; the student did not generate those tokens. OPD evaluates the response written by the student, so only response-token rewards enter training.

Positions 0 and 1 of `logps` predict the prompt tokens `answer` and `is`. We can compute teacher-student differences at those positions, but they still evaluate prompt text, so they are discarded. Positions 2 and 3 predict `4` and `.`, which were generated by the student, so those rewards are kept.

On a printed test, the question is already provided and the student writes only the answer. The score is based on what the student writes.
...
(teacher_logps - student_logps)[0, 2:]
                                 ...
                                 └──── select sample 0 in the batch
```

We can now substitute concrete scores and execute the slice:

```python
# The four positions predict "answer," "is," "4," and "."
...
Placing these values back under their token positions shows exactly what the slice removes:
...
| Reward after subtraction |    -0.20 |   +0.30 | **+0.70** | **-0.10** |
| Result of `[0, 2:]`      |  discard | discard |  **keep** |  **keep** |

The two outer brackets of `all_rewards` correspond to shape `[1, 4]`: the batch contains one sample, and that sample has four next-token rewards. Index `0` first selects that sample, changing the two-dimensional `[[...]]` tensor into a one-dimensional `[...]` tensor. Slice `2:` then removes positions 0 and 1, retaining positions 2 and 3—the rewards for `4` and `.`.

The final `tensor([0.7000, -0.1000])` contains two elements. The `+0.70` for `4` means the teacher assigns that token more probability than the student. The `-0.10` for `.` means the teacher assigns it slightly less. The prompt rewards `-0.20` and `+0.30` have been removed and do not participate in response training.

When processing a full equal-length batch, the first index can be changed to `:`. When prompt lengths differ, each sample needs a row of True/False values marking which positions belong to the response. This row is called a response mask.
